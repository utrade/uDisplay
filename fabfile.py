...
def prod():
    ...
    env.remote = 'origin'
    env.branch = 'prod'
    env.hosts = ['prod.udisplay.com']
    env.dotenv_path = '/home/ubuntu/uDisplay/.env'
...
def tornado():
    ...
    install_requirements()
    with virtualenv():
        local('python socket_server.py')


def push_server():
    '''Start a mock push server for testing'''
    install_requirements()
    with virtualenv():
        local('cd udisplay/base/utils && python push_server.py')
